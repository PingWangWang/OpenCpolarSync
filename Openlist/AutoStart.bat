...
:: 验证 OpenlistGuard.ps1 存在
if not exist "OpenlistGuard.ps1" (
    echo 错误：OpenlistGuard.ps1 未找到。
    echo 请从 Openlist 目录运行此脚本。
    pause
    exit /b 1
)
...
:: 窗口隐藏由 OpenlistGuard.ps1 内部自处理，避免 AV 检测
...
if exist "%LNK%" (
    echo.
    echo 开机自启已添加！
    echo OpenlistGuard 将在下次登录时自动启动。
    echo 快捷方式：%LNK%
) else (
    echo 错误：创建快捷方式失败。
)
